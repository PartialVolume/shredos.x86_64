...
for ((i=0; i<cores; i++)); do
    ...
    # BusyBox-compatible: first get a free loopdev, then attach the file
    ld=$(losetup -f) || {
        echo "create_ramloops: losetup -f failed" >&2
        exit 1
    }

    losetup "$ld" "$img" || {
        echo "create_ramloops: failed to attach $img to $ld" >&2
        exit 1
    }
